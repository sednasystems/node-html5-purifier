...
function parse(htmlInput, prefix, postfix, sanitizerLogger, messageDir, cb) {
  async.waterfall([
    ...
    function(cb) {
      sanitizerLogger && sanitizerLogger.debug(`MessageDir: ${messageDir} HTML Parser: Remove invalid pcdata`);
    ...
    function(pcdataValidated, cb) {
      sanitizerLogger && sanitizerLogger.debug(`MessageDir: ${messageDir} HTML Parser: google caja sanitizer`);
      sanitizer.sanitize(pcdataValidated, cb);
    },
    // remove all tags that are not on the tags whitelist
    function(sanitizedHtml, cb) {
      sanitizerLogger && sanitizerLogger.debug(`MessageDir: ${messageDir} HTML Parser: remove all tags that are not on the tags whitelist`);
    ...
    function(tagValidatedHtml, cb) {
      sanitizerLogger && sanitizerLogger.debug(`MessageDir: ${messageDir} HTML Parser: remove all tags that are not on the attributes whitelist`);
    ...
    function(attributeSanitizedHtml, cb) {
      if (typeof prefix !== 'undefined' || typeof postfix !== 'undefined') {
        sanitizerLogger && sanitizerLogger.debug(`MessageDir: ${messageDir} "HTML Parser: add namespace`);
...
function parsePurified(htmlInput, prefix, postfix, deSanitizerLogger, messageDir, cb) {
  async.waterfall([
    // remove invalid pcdata, ie) <![if mso ...]>
    function(cb) {
      deSanitizerLogger && deSanitizerLogger.debug(`MessageDir: ${messageDir} HTML Parser: Remove invalid pcdata`);
      pcdataValidator.filter(htmlInput, cb);
    },
    // sanitize - caja sanitizes, tag balances, and converts html entities
    function(pcdataValidated, cb) {
      deSanitizerLogger && deSanitizerLogger.debug(`MessageDir: ${messageDir} HTML Parser: google caja sanitizer`);
      sanitizer.sanitize(pcdataValidated, cb);
    },
    // remove all tags that are not on the tags whitelist
    function(sanitizedHtml, cb) {
      deSanitizerLogger && deSanitizerLogger.debug(`MessageDir: ${messageDir} HTML Parser: remove all tags that are not on the tags whitelist`);
      tagValidator.validate(sanitizedHtml, cb);
    },
    // remove all tags that are not on the attributes whitelist
    function(tagValidatedHtml, cb) {
      deSanitizerLogger && deSanitizerLogger.debug(`MessageDir: ${messageDir} HTML Parser: remove all tags that are not on the attributes whitelist`);
      attributeValidator.validate(tagValidatedHtml, cb);
    },
    function(attributeSanitizedHtml, cb) {
      ...
      if (typeof prefix !== 'undefined' || typeof postfix !== 'undefined') {
        ...
        async.whilst(function() {
          ...
          deSanitizerLogger && deSanitizerLogger.debug(`MessageDir: ${messageDir} HTML Parser: strip namespace`);
