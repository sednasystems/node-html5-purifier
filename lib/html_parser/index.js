...
function parse(htmlInput, prefix, postfix, cb) {
  async.waterfall([
    ...
    function(cb) {
      sanitizerLogger.info("HTML Parser: Remove invalid pcdata");
    ...
    function(pcdataValidated, cb) {
      sanitizerLogger.info("HTML Parser: google caja sanitizer");
      sanitizer.sanitize(pcdataValidated, cb);
    },
    // remove all tags that are not on the tags whitelist
    function(sanitizedHtml, cb) {
      sanitizerLogger.info("HTML Parser: remove all tags that are not on the tags whitelist");
    ...
    function(tagValidatedHtml, cb) {
      sanitizerLogger.info("HTML Parser: remove all tags that are not on the attributes whitelist");
    ...
    function(attributeSanitizedHtml, cb) {
      if (typeof prefix !== 'undefined' || typeof postfix !== 'undefined') {
        sanitizerLogger.info("HTML Parser: add namespace");
...
function parsePurified(htmlInput, prefix, postfix, cb) {
  async.waterfall([
    // remove invalid pcdata, ie) <![if mso ...]>
    function(cb) {
      deSanitizerLogger.info("HTML Parser: Remove invalid pcdata");
      pcdataValidator.filter(htmlInput, cb);
    },
    // sanitize - caja sanitizes, tag balances, and converts html entities
    function(pcdataValidated, cb) {
      deSanitizerLogger.info("HTML Parser: google caja sanitizer");
      sanitizer.sanitize(pcdataValidated, cb);
    },
    // remove all tags that are not on the tags whitelist
    function(sanitizedHtml, cb) {
      deSanitizerLogger.info("HTML Parser: remove all tags that are not on the tags whitelist");
      tagValidator.validate(sanitizedHtml, cb);
    },
    // remove all tags that are not on the attributes whitelist
    function(tagValidatedHtml, cb) {
      deSanitizerLogger.info("HTML Parser: remove all tags that are not on the attributes whitelist");
      attributeValidator.validate(tagValidatedHtml, cb);
    },
    function(attributeSanitizedHtml, cb) {
      ...
      if (typeof prefix !== 'undefined' || typeof postfix !== 'undefined') {
        ...
        async.whilst(function() {
          ...
          deSanitizerLogger.info("HTML Parser: strip namespace");
