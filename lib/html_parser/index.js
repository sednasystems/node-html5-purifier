 ...
 * @pattern facade
 */
function parse(htmlInput, prefix, postfix, sanitizerLogger, messageId, cb) {
  async.waterfall([
    // remove invalid pcdata, ie) <![if mso ...]>
    function(cb) {
      sanitizerLogger && sanitizerLogger.info(`MessageId: ${messageId} HTML Parser: Remove invalid pcdata`);
    ...
    function(pcdataValidated, cb) {
      sanitizerLogger && sanitizerLogger.info(`MessageId: ${messageId} HTML Parser: google caja sanitizer`);
      sanitizer.sanitize(pcdataValidated, cb);
    },
    // remove all tags that are not on the tags whitelist
    function(sanitizedHtml, cb) {
      sanitizerLogger && sanitizerLogger.info(`MessageId: ${messageId} HTML Parser: remove all tags that are not on the tags whitelist`);
    ...
    function(tagValidatedHtml, cb) {
      sanitizerLogger && sanitizerLogger.info(`MessageId: ${messageId} HTML Parser: remove all tags that are not on the attributes whitelist`);
    ...
    function(attributeSanitizedHtml, cb) {
      if (typeof prefix !== 'undefined' || typeof postfix !== 'undefined') {
        sanitizerLogger && sanitizerLogger.info(`MessageId: ${messageId} "HTML Parser: add namespace`);
...
function parsePurified(htmlInput, prefix, postfix, deSanitizerLogger, messageId, cb) {
  async.waterfall([
    // remove invalid pcdata, ie) <![if mso ...]>
    function(cb) {
      deSanitizerLogger && deSanitizerLogger.info(`MessageId: ${messageId} HTML Parser: Remove invalid pcdata`);
      pcdataValidator.filter(htmlInput, cb);
    },
    // sanitize - caja sanitizes, tag balances, and converts html entities
    function(pcdataValidated, cb) {
      deSanitizerLogger && deSanitizerLogger.info(`MessageId: ${messageId} HTML Parser: google caja sanitizer`);
      sanitizer.sanitize(pcdataValidated, cb);
    },
    // remove all tags that are not on the tags whitelist
    function(sanitizedHtml, cb) {
      deSanitizerLogger && deSanitizerLogger.info(`MessageId: ${messageId} HTML Parser: remove all tags that are not on the tags whitelist`);
      tagValidator.validate(sanitizedHtml, cb);
    },
    // remove all tags that are not on the attributes whitelist
    function(tagValidatedHtml, cb) {
      deSanitizerLogger && deSanitizerLogger.info(`MessageId: ${messageId} HTML Parser: remove all tags that are not on the attributes whitelist`);
      attributeValidator.validate(tagValidatedHtml, cb);
    },
    function(attributeSanitizedHtml, cb) {
      ...
      if (typeof prefix !== 'undefined' || typeof postfix !== 'undefined') {
        ...
        async.whilst(function() {
          ...
          deSanitizerLogger && deSanitizerLogger.info(`MessageId: ${messageId} HTML Parser: strip namespace`);
