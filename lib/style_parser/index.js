 ...
 * @pattern facade
 */
function parse(cssInput, prefix, postfix, sanitizerLogger, messageId, cb) {
  async.waterfall([
    // remove everything except style tag contents
    function(cb) {
      sanitizerLogger && sanitizerLogger.info(`MessageId: ${messageId} StyleParser: remove everything except style tag contents`);
      sanitizer.sanitize(cssInput, cb);
    },
    // namespace sanitized style tag contents
    function(sanitizedHtml, cb) {
      sanitizerLogger && sanitizerLogger.info(`MessageId: ${messageId} StyleParser: add namespace`);
      namespacer.namespace(sanitizedHtml, prefix, postfix, cb);
    },
    // add style tags
    function(nameSpacedHtml, cb) {
      sanitizerLogger && sanitizerLogger.info(`MessageId: ${messageId} StyleParser: add style tags`);
 ...
 * @pattern facade
 */
function parsePurified(cssInput, prefix, postfix, deSanitizerLogger, messageId, cb) {
  var strippedStyles;
  async.waterfall([
    // remove everything except style tag contents
    function(cb) {
      deSanitizerLogger && deSanitizerLogger.info(`MessageId: ${messageId} StyleParser: remove everything except style tag contents`);
      sanitizer.sanitize(cssInput, cb);
    },
    // strip namespace
    function(sanitizedHtml, cb) {
      deSanitizerLogger && deSanitizerLogger.info(`MessageId: ${messageId} StyleParser: strip namespace`);
      if ((typeof prefix !== 'undefined' || typeof postfix !== 'undefined') && sanitizedHtml !== '') {
        namespacer.stripNamespace(sanitizedHtml, prefix, postfix, function(err, stripped) {
          deSanitizerLogger && deSanitizerLogger.info(`MessageId: ${messageId} StyleParser: add style tags`);
